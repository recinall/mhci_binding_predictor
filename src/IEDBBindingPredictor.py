...
class IEDBBindingPredictor:
    ...
    def generate_variants(self, sequence_pattern):
        """Genera varianti da pattern (es. A[CD]E[FY]GH)"""
        variants = []
        ...
        def generate_combinations(current_seq, remaining_pattern):
            ...
            # Se il carattere inizia con '[', estrai le opzioni tra parentesi
        ...
        if not variants:
            # Se non ci sono varianti, considera il pattern come sequenza diretta
            variants = [sequence_pattern]
            
        logger.info(f"Generate {len(variants)} varianti dalla sequenza pattern")
        return variants
...
if __name__ == "__main__":
    ...
    # Initialize predictor with default CSV format (comma separator, dot decimal)
    predictor = IEDBBindingPredictor(output_dir="./iedb_results")

    predictor.set_csv_format(csv_separator=";", decimal_separator=",")
    
    # Example of analyzing peptides with both netmhcpan_el and netmhcpan_ba methods
    print("Analyzing peptides with both netmhcpan_el and netmhcpan_ba methods...")
    results = predictor.analyze_peptides_batch(peptides, alleles)
    
    for allele, df in results.items():
        if not df.empty:
            print(f"\nResults for {allele}:")
            print(df[['peptide', 'score', 'percentile_rank', 'ic50', 'immunogenicity']].head())
            
            # Filter for strong binders (both good binding score and IC50)
            strong_binders = predictor.filter_binders(
                df, 
                threshold=0.5,           # Good EL score (higher is better)
                rank_threshold=2.0,      # Low percentile rank (lower is better)
                ic50_threshold=500,      # Low IC50 (lower is better)
                immunogenicity_threshold=0.1  # Good immunogenicity (higher is better)
            )
            
            if not strong_binders.empty:
                print(f"\nFound {len(strong_binders)} strong binders for {allele}:")
                print(strong_binders[['peptide', 'score', 'percentile_rank', 'ic50', 'immunogenicity']])
                
                # Save strong binders
                strong_csv = os.path.join(predictor.output_dir, f"strong_binders_{allele.replace('*', '').replace(':', '')}.csv")
                predictor.save_to_csv(strong_binders, strong_csv)
                print(f"Strong binders saved to: {strong_csv}")
            else:
                print(f"No strong binders found for {allele}")
...
"""The MHCI binding predictions were made on 4/9/2025 using the IEDB analysis resource NetMHCpan (ver. 4.1) tool [1].
